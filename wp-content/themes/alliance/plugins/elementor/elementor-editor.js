...
jQuery( document ).ready(
	function() {
		...
		alliance_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="alliance_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.alliance_post_editor' );
						if ( link.length > 0 ) {
							alliance_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.alliance_post_editor', function(e) {
						alliance_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('alliance_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function alliance_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
