...
jQuery( document ).on( 'action.ready_alliance', function() {
	...
	$document.on( 'action.resize_alliance', function() {
		_list_products_height = $list_products.length > 0 ? $list_products.height() : 0;
	} );
	...
	// Change display mode
	//--------------------------------
	jQuery( '.woocommerce,.woocommerce-page' ).on( 'click', '.alliance_shop_mode_buttons a', function(e) {
		var $self = jQuery(this),
			mode  = $self.hasClass( 'woocommerce_thumbs' ) ? 'thumbs' : 'list';
		alliance_set_cookie( 'alliance_shop_mode', mode, 365 * 24 * 60 * 60 * 1000 );   // 1 year
	...
	// On first run
	alliance_woocommerce_add_inc_dec();
	...
	// Add buttons inc/dec
	function alliance_woocommerce_add_inc_dec() {
		jQuery( '.woocommerce div.quantity:not(.inited),.woocommerce-page div.quantity:not(.inited)' ).each( function() {
			...
			$self
				...
				.on( 'click', '>span', function(e) {
					alliance_woocomerce_inc_dec_click( jQuery( this ) );
					e.preventDefault();
					return false;
				} );
		} );
	...
	function alliance_woocomerce_inc_dec_click(button) {
	...
	$document.on( 'action.scroll_alliance', function(e) {
		...
		if ( $list_products.length > 0 && $list_products.offset().top + _list_products_height < alliance_window_scroll_top() + alliance_window_height() * 1.5 ) {
	...
	jQuery( document.body ).on( 'updated_wc_div', function() {
		...
		alliance_create_observer( 'update_woocommerce_cart', table_cart.parents('form').eq(0), function( mutationsList ) {
			for ( var mutation of mutationsList ) {
				if ( mutation.type == 'childList' || mutation.type == 'subtree' ) {
					alliance_remove_observer( 'update_woocommerce_cart' );
					table_cart = jQuery( 'table.cart' );
					alliance_woocommerce_update_cart( 'update' );
					break;
				}
			}
		} );
	...
	if ( table_cart.length > 0 ) {
		alliance_woocommerce_update_cart( 'init' );
	}

	// Resize action
	$window.on( 'resize', function() {
		if ( table_cart.length > 0 ) {
			alliance_woocommerce_update_cart( 'resize' );
		}
	} );

	// Update cart display
	function alliance_woocommerce_update_cart(status){
		setTimeout(
			function() {
				var w = window.innerWidth;
				if (w == undefined) {
					w = alliance_window_width() + ( alliance_window_height() < $document.height() || alliance_window_scroll_top() > 0 ? 16 : 0 );
				}
				...
				if ( w >= ALLIANCE_STORAGE['mobile_layout_width'] && status == 'resize' && jQuery( 'table.cart .mobile_cell' ).length > 0) {
					if ( tbl.length > 0 ) {
