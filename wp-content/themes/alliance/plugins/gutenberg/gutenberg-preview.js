...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			alliance_create_observer( 'alliance-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				alliance_gutenberg_editor_init();
			} );
		} else {
		 	alliance_gutenberg_editor_init();
		}
	...
	function alliance_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'alliance_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'alliance_inited' ) ) {
	...
	// Init on page load
	function alliance_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = alliance_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + ALLIANCE_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + ALLIANCE_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in ALLIANCE_STORAGE['override_classes'] ) {
				$obj.addClass( ALLIANCE_STORAGE['override_classes'][i].replace( '%s', ALLIANCE_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( alliance_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && ALLIANCE_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'alliance_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			alliance_remove_observer( 'alliance-check-editor-styles-wrapper' );
			alliance_create_observer( 'alliance-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'alliance_inited' );
	}
} );
